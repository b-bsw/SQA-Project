...
เอกสารอธิบายการทำงาน โครงสร้าง และคำสั่งสำหรับรันสคริปต์ **`script/Round1/generate_deepseek_tests.py`** เพื่อสร้าง JUnit Test Suite อัตโนมัติด้วยโมเดล **`deepseek-v4-flash`** ผ่าน KKU IntelSphere API (`https://gen.ai.kku.ac.th/api/v1/chat/completions`) สำหรับโปรเจกต์ Defects4J
...
```bash
python script/Round1/generate_deepseek_tests.py --status
```
...
```bash
python script/Round1/generate_deepseek_tests.py --project Closure_28 --plan
```
...
```bash
python script/Round1/generate_deepseek_tests.py --project Chart_1 --dry-run
```

### 11.4 ตรวจสอบโควต้าจริงของแต่ละคีย์จากเซิร์ฟเวอร์
```bash
python script/Round1/generate_deepseek_tests.py --check-quota
```

### 11.5 รันจริงเฉพาะโปรเจกต์ที่ต้องการ
```bash
python script/Round1/generate_deepseek_tests.py --project Closure_28
```

### 11.6 รันจริงพร้อมแสดงผลการสตรีมสดลงหน้าจอ
```bash
python script/Round1/generate_deepseek_tests.py --project Chart_1 -v
```

### 11.7 รันผ่าน Shell Script Wrapper บน WSL / Linux
```bash
bash script/Round1/generate_deepseek_tests.sh --project Closure_28 --status
```
...
```bash
python3 script/Round1/run_deepseek_parallel.py --workers 4
```

หากต้องการปิด **งบ token ภายในเครื่อง** ของ worker ให้ใส่ `--no-budget` เช่น `python3 script/Round1/run_deepseek_parallel.py --workers 5 --no-budget` ตัวเลือกนี้ไม่ยกเลิกโควต้า 1,000,000 tokens/วันต่อคีย์ที่ API กำหนด เมื่อโควต้าฝั่งเซิร์ฟเวอร์หมด worker นั้นยังหยุดและสามารถรันคำสั่งเดิมต่อได้หลังโควต้ารีเซ็ต

เฉพาะ Cli และ Chart หรือดูการแบ่งงานแบบ offline ก่อนรัน:

```bash
python3 script/Round1/run_deepseek_parallel.py --workers 2 --projects Cli Chart
python3 script/Round1/run_deepseek_parallel.py --workers 2 --projects Cli Chart --dry-run
```
...
```bash
python script/Round1/test_generate_deepseek_tests.py
```
