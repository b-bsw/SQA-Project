...
def build_prompt(template: str, source_code: str, dependencies: str = "",
    ...
    dep_spec = dependencies.strip() if dependencies else "No project dependency metadata provided."
...
def load_progress_snapshot(state_file: Path, output_dir: Path, include_shards: bool = True) -> tuple[dict, int]:
    ...
    shard_count = 0
    if include_shards:
        for shard in sorted((output_dir / "state" / "Round1" / "shards").glob("*.json")):
...
def main(cli_args=None):
    ...
    parser.add_argument("--state-file", type=Path, help="ไฟล์สถานะเฉพาะงาน (ค่าเริ่มต้น: Deepseek-flash-v4/state/Round1/generator_state.json)")
    ...
    parser.add_argument("--include-dependencies", action="store_true",
                        help="Include project dependency metadata and sibling signatures in the prompt (Round 2)")
    ...
    state_file = args.state_file or workspace_dir / "Deepseek-flash-v4" / "state" / "Round1" / "generator_state.json"
    ...
    try:
        for idx, task in enumerate(tasks_to_run, 1):
            ...
            dependency_spec, signatures = (collect_dependency_context(src_path, proj_name, workspace_dir)
                                           if args.include_dependencies else ("", ""))
            ...
            if needs_retry and api_result.get("content"):
                ...
                first_usage = api_result.get("usage") or {}
                ...
                for token_field in ("prompt_tokens", "completion_tokens", "total_tokens"):
                    first_value = first_usage.get(token_field)
                    retry_value = retry_usage.get(token_field)
                    if isinstance(first_value, (int, float)) and isinstance(retry_value, (int, float)):
                        combined_usage[token_field] = first_value + retry_value
                    elif isinstance(retry_value, (int, float)):
                        combined_usage[token_field] = retry_value
                retry_result["usage"] = combined_usage
                combined_elapsed = round(api_result.get("elapsed", 0) +
                                         retry_result.get("elapsed", 0), 2)
                retry_result["elapsed"] = combined_elapsed
                api_result["usage"] = combined_usage
                api_result["elapsed"] = combined_elapsed
