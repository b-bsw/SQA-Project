...
def run_generator(target, slot, key, args):
    ...
    command = [sys.executable, "-u", str(DEEPSEEK / "Code/deepseek/Round1/generate_deepseek_tests.py"),
               ...
               "--junit", "junit4", "--delay", "0", "--include-dependencies"]
    if args.env_file:
...
def main(argv=None):
    ...
    parser.add_argument("--status", action="store_true", help="Read saved generation and Result2 progress; no API or writes")
    ...
    if min(args.workers, args.timeout,
    ...
    if sum((args.execute, args.publish_ready, args.status,
    ...
    if args.publish_ready:
        ...
        for entry in selected:
            ...
            if files:
                ...
                published += 1
        print(f"Published {published} staged targets to TestCode2.", flush=True)
        return 0
    if not args.execute:
    ...
    if not selected:
        print("No pending generation. Use Code/run_deepseek_tests.py separately when you want test results.", flush=True)
        return 0
    ...
    print(f"Regeneration committed: {len(manifest['targets']) - remaining}; remaining: {remaining}",
          flush=True)
    print("Generation phase finished. Run Code/run_deepseek_tests.py separately to create Result2.", flush=True)
