...
def restore_pilot(deepseek):
    ...
    baseline = deepseek / "state" / "Round2" / "baseline" / PILOT
...
def migrate(deepseek):
    ...
    old_work = new_code / ".work"
    new_work = deepseek / "state" / "Round2"
    ...
    if not old_work.is_dir() and not new_work.is_dir():
        raise RuntimeError("Round2 state is missing")
    ...
    if old_work.is_dir():
        ...
        if new_work.exists():
            raise RuntimeError(f"Cannot move state over {new_work}")
        old_work.rename(new_work)

    for path in (new_work / "shards").glob("*.json"):
        ...
        for record in data.values():
            ...
            updated = old.replace("Deepseek-flash-v4/TestCode2/.work/",
                                  "Deepseek-flash-v4/state/Round2/")
            updated = updated.replace("Deepseek-flash-v4\\TestCode2\\.work\\",
                                      "Deepseek-flash-v4\\state\\Round2\\")
            if updated != old:
