...
class RegenerationTests(unittest.TestCase):
    ...
    def test_status_counts_progress_without_side_effects(self):
        ...
        for i, status in enumerate(["COMMITTED", "COMMITTED", "FAILED", "PAUSED"], 1):
            (work / "status" / f"Sample_{i}.json").write_text(
                ...
                encoding="utf-8")
        (work / "shards" / "Sample_1.json").write_text(json.dumps({
        ...
        with patch.object(regen, "REGEN", work), patch.object(regen, "RESULT", results), \
            ...
            self.assertIn("Result2 reports: 1/2 committed targets", summary)
        ...
        for mock in (keys, post, save, process):
    ...
    def test_status_empty_cohort_and_conflicting_modes(self):
        ...
        for flags in [["--execute"], ["--publish-ready"],
    ...
    def test_execute_only_generates_tests(self):
        manifest = {"targets": [{"target": "Sample_1", "sources": ["Widget.java"]}]}
    ...
    def test_api_failure_is_not_mislabeled_token_limit(self):
        ...
        with patch.object(gen, "__file__", str(self.root / "Deepseek-flash-v4/Code/deepseek/Round1/generate_deepseek_tests.py")), \
             patch.object(gen, "get_all_api_keys", return_value=["dummy-key"]), \
    ...
    def test_wrapper_reports_source_failure_in_terminal_and_status(self):
        target = "Sample_2"
        work = self.root / "work"
        state_file = work / "shards" / f"{target}.json"
    ...
    def test_commit_publishes_only_to_testcode2(self):
        ...
        regeneration = deepseek / "state" / "Round2"
        staging = regeneration / "staging" / "Sample_2" / "Sample_2_buggy"
        ...
        with patch.object(regen, "REGEN", regeneration), \
        ...
        self.assertEqual(json.loads((regeneration / "status" /
    ...
    def test_layout_migration_is_reversible_and_idempotent(self):
        ...
        baseline = deepseek / "state" / "Round2" / "baseline" / "Mockito_29"
        ...
        self.assertTrue((deepseek / "state" / "Round2").is_dir())
